...
// Add this temporarily to app.ts then visit http://localhost:3000/seed
app.get('/seed', async (c) => {
  try {
    await userDatabase.createUser("test@example.com", "Test User", "password123");
    return c.json({ message: "User created! You can now login." });
  } catch (e) {
